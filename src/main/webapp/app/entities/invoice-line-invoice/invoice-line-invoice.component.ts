...
@Component({
    selector: 'inv-invoice-line-invoice',
    templateUrl: './invoice-line-invoice.component.html'
})
export class InvoiceLineInvoiceComponent implements OnInit, OnDestroy {

currentAccount: any;
    invoiceLines: InvoiceLineInvoice[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/invoice-line-invoice'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/invoice-line-invoice', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: InvoiceLineInvoice) {
        return item.id;
    }
