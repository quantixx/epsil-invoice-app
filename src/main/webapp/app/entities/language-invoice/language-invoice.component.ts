...
@Component({
    selector: 'inv-language-invoice',
    templateUrl: './language-invoice.component.html'
})
export class LanguageInvoiceComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/language-invoice'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/language-invoice', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: LanguageInvoice) {
        return item.id;
    }
