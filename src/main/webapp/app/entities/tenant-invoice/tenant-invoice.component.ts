...
@Component({
    selector: 'inv-tenant-invoice',
    templateUrl: './tenant-invoice.component.html'
})
export class TenantInvoiceComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/tenant-invoice'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/tenant-invoice', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: TenantInvoice) {
        return item.id;
    }
