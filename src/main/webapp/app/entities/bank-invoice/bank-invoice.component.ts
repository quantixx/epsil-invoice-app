...
@Component({
    selector: 'inv-bank-invoice',
    templateUrl: './bank-invoice.component.html'
})
export class BankInvoiceComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/bank-invoice'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/bank-invoice', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BankInvoice) {
        return item.id;
    }
