...
@Component({
    selector: 'inv-quotation-invoice',
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/quotation-invoice'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/quotation-invoice', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: QuotationInvoice) {
        return item.id;
    }
